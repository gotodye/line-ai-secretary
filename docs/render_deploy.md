...
| `CRON_SECRET` | 選填 | 晨間簡報端點的通行密碼；未設定則該端點停用 |
...
`/cron/brief` 會為每位已連結 Google 的使用者產生簡報並主動推播到 LINE。它自己不含排程器，要由外部服務定時呼叫。

Render 免費方案沒有 Cron Job，用 [cron-job.org](https://cron-job.org)（免費）即可：

1. 註冊後建立 Cronjob
2. URL 填 `https://<你的服務>.onrender.com/cron/brief`
3. 執行時間設每天早上（例如 07:30，時區選 Asia/Taipei）
4. 展開 **Advanced** → **Headers**，新增 `X-Cron-Secret` = 你的 `CRON_SECRET`

密碼也可以用查詢參數 `?secret=xxx`，但放在標頭比較不會被記進各種存取紀錄。

未帶密碼或密碼錯誤一律回 401；未設定 `CRON_SECRET` 時端點回 503，不會對外開放。

端點收到請求後立刻回 `202 Accepted`，簡報在背景產生並推播。一位使用者的簡報要跑 20 秒以上，多位使用者就是好幾十秒，遠超過排程服務的請求逾時；**看到 202 只代表已受理，不代表簡報已送出**，實際結果請看 Render 日誌的「晨間簡報結果」。

端點有冪等保護：同一位使用者當天已成功寄送就會跳過，所以可以安全地一天排多個觸發時間當備援（見下方 GitHub Actions）。手動測試要強制重寄時加 `?force=1`。

### Keep-alive（晨間簡報的前提）

免費方案閒置 15 分鐘就休眠，冷啟動要 30–60 秒。早上第一個請求打進來時服務通常是睡的，排程服務會先逾時。

所以要另外加一個 keep-alive 排程：每 10 分鐘 GET 一次 `/health`（不需密碼，這支不碰 Upstash）。沒有這個，晨間簡報會很不穩定。

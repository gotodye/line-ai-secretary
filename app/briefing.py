...
3. 呼叫 list_recent_emails 查未讀信件（query 用 is:unread newer_than:2d，max_results=8）
...
- 「信件」段落：**逐封列出寄件者與主旨（最多 6 封），絕對不要只寫「N 封未讀」了事**。
  就算是廣告、促銷、電子報也照列，讓我自己判斷，不要幫我過濾掉。
  若有你判斷特別重要的（帳單、工作、真人來信），排在最前面。
  真的一封未讀都沒有時，才省略這個段落。
